...
vtkRenderer, vtkRenderWindow, vtkCamera,vtkWindowToImageFilter, vtkBMPWriter,vtkImageShiftScale)
...
class arcsim_expert:
    ...
    def save_frame_image(self, path):
        vss, nss, iss = self.save_frame_mesh()
        #render
    ...
    def save_frame_depth(self, path):
        ...
        writer.Write()
    def save_frame(self, path, image, depth=False):
        self.save_frame_vtk(path+".vtk")
        if image:
            self.save_frame_image(path+".png")
        if depth:
            self.save_frame_depth(path+".bmp")
    ...
    def illustrate_expert_python(self, x, y, delta, \
                                 path, nr_frame, nr_pass, expert, image, depth):
        if os.path.exists(path):
            shutil.rmtree(path)
        os.mkdir(path)
        ...
        for p in xrange(nr_pass):
            ...
            for i in xrange(nr_frame):
                ...
                self.save_frame(path+"/frm"+str(j),image,depth)
                j=j+1
...
if __name__== "__main__":
    ...
    expert.illustrate_expert_python(1,2,0.01,"./python_sheet_expert_output",100,5,expert.expert_twist,True,True)
